...
int main()
{
	ifstream inImgPath(data_dir + "chess.txt");    
	...
	ofstream fout(data_dir + "caliberation_result.txt");   //保存标定结果的文件
	...
	while (image_num < imgList.size())
	{
        ...
        auto size = findChessboardCorners(imageInput, pattern_size, corner_points_buf);
        cout<<"==> "<< size << endl;
		if (size == 0)
		{
...
//             continue;
	...
	for (int i = 0; i < image_num; i++)
	{
		...
		imshow("before", src_image);
		imshow("after", new_image);
		...
		cv::imwrite(data_dir + imageFileName, new_image);
