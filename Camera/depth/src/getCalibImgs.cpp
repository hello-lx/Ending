...
void saveImage(const Mat &depth, const Mat &color, int index);
...
//             IplImage image = previewImg;
		...
		if(key==110 && check(frame)){ // key = n
            saveImage(previewImg, frame, index++);
        }
	}
...
void saveImage(const Mat &depth, const Mat &color, int index){
    ...
    rect.x = depth.cols/2;
    rect.y = 0;
    rect.width = depth.cols / 2;
    rect.height = depth.rows;
    
    Mat realD = depth(rect);
    
    if(realD.data != NULL)
        imwrite(ssD.str(), realD);
