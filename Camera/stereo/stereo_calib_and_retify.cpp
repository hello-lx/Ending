...
const  float chessboardSquareSize = 20.0f; // 12.5f;  //每个棋盘格方块的边长 单位 为 mm
...
int main()
{
	...
	Mat before_rectify = imread("../data/002_left.png");
...
// 	imshow("校正后", canvas); //显示画绿线的校正前图像
	imshow("retify_before", before_rectify); //显示画绿线的校正后图像
	imshow("retify_after", canvas); //显示画绿线的校正前图像
