...
main(int argc, char** argv)
{
    string pcdPath;
    if(argc < 2)
//         pcdPath="/home/yuan/XSpace/Ending/DenseSurfelMapping/surfel_fusion/results/kitti/kitti00_loop.PCD";
        pcdPath="/home/yuan/XSpace/Ending/pcl_test/rabbit.pcd";
    else{
        pcdPath = "/home/yuan/Xstudio/pcl/pcl-pcl-1.9.1/test/" + (string)argv[1];
    }
    
    // 将一个xyz点类型的pcd文件打开并存储到对象PointCloud中
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PCLPointCloud2 cloud_blob;
    pcl::io::loadPCDFile(pcdPath, cloud_blob); //加载table_scene_lms400_downsampled.pcd文件
    ...
    string result = "./result.ply";
    pcl::io::savePLYFileBinary(result.c_str(), triangles);
    gp3.setSearchRadius(25); //设置连接点之间的最大距离（即为三角形最大边长）为0.025
    //设置各参数特征值
    gp3.setMu(2500);                         //设置被样本点搜索其邻近点的最远距离为2.5，为了适应点云密度的变化
