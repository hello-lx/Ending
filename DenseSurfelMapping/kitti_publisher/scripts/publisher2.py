...
if __name__ == '__main__':
    ...
    path_name = "/home/yuan/XSpace/Ending/Camera/AstraSDK-v2.0.16-Linux/data"
    image_index = 0
    rate = rospy.Rate(5)
    ...
    while True:
        print(image_index)
        #left_img_path = path_name + "/%05d_color.png"%image_index
        #right_img_path = path_name + "/%05d_color.png"%image_index
        #depth_path = path_name + "/%05d_depth.png" % image_index
        img_path = path_name + "/%05d.png"%image_index
        #if (not os.path.isfile(left_img_path)) or (not os.path.isfile(right_img_path)):
            #break
        if not os.path.isfile(img_path): break
        ...
        #depth_visual = cv2.imread(depth_path, 0)
        image = cv2.imread(img_path, 0)
        left_img = image[:460, 610:-1]
        right_img = image[:460, 610:-1]
        depth_np = image[:460, :610].astype(np.float32)
        depth_visual = image[:460, :610]
