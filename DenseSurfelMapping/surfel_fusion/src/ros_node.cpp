...
int main(int argc, char **argv)
{
 ...
 string save_name = "/home/yuan/XSpace/Ending/DenseSurfelMapping/surfel_fusion/results/kitti/kitti00_loop";
//   if(nh.getParam("save_name", save_name))
    if(true)
  {
    string pcd_name = save_name + ".PCD";
    string mesh_name = save_name + "_mesh.PLY";
    std::cout << "<==" << "save_name: " << pcd_name << "==>" << std::endl;
    std::cout << "<==" << "save_name: " << mesh_name << "==>" << std::endl;
    surfel_map.save_cloud(pcd_name);
    surfel_map.save_mesh(mesh_name);
  }else{
    std::cout << "<==" << "Cloud not be saved!" << "==>" << std::endl;
}
